...
### Fonction retournant au moins les k plus proches utilisateurs de l’utilisateur user_index ansi que leurs poids

On cherche ici a récupérer tout les amis les plus proches de l'utilisateur et quantifier leur similarité à l'utilisateur visé. C'est ce que leur "poids" représente
Le k représente le nombre d'amis minimum que retourne la fonction. si l'utilisateur a moins d'amis que cela, on cherche les utilisateurs les plus proches dans tout la matrice de vote
```{r}
Closest_friends <- function(user_index,k,alpha){
  ...
  if (n_rest_friends <= 0){
    #sim <- cosinus.vm(user,user_adj.sparse[friends_id,])
...
Friends <- function(user_index,alpha){
  user <- user_adj.sparse[user_index,]
  friends_id <- colnames(user_adj.sparse)[user!=0]
  friends <- user_adj.sparse[friends_id,]
...
exemple: pour l'utilisateur 2:
```{r}
as.data.frame(as.matrix(Friends(2,0)))
```

## Fonction recommandant n artistes à l'utilisateur *user_index*

Ici, on simule un algorithme knn, en se concentrant seulement sur ses k plus proche amis et non pas voisins
```{r}
...
exemple pour l'utilisateur 2:
```{r}
data.frame(metric = c('nombre de bonne recommandation', 'nombre d\'écoute de l\'utilisateur pour les artistes recommandé ', 'nombre d\'amis', 'nombre total d\'écoute', 'temps'),
...
```
 exemple pour 40 amis minimum, et 50 recommandation
```{r}
...
Nous allons maintenant essayer plusieurs valeurs d'hyper-parametres
```{r}
library(parallel)
