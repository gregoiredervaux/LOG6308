...
data.frame( names = c("acc PHY", "acc PSY", "acc Tot"), mean = as.double(k_mean_acc))
```
On peut voir que l'algorithme K-mean n'arrive pas a départager les deux clusters
En effet, on peut voir que le cluster PHY a été attribué à presque tout les cours, jusqu'a avoir une accuracy de 100% sur cette classe
Cela est due au fait que la classe PHY est beaucoup plus grande que la classe PSY en nombre de cours
...
data.frame( names = c("acc PHY", "acc PSY", "acc Tot"), mean = as.double(k_mean_acc.equi))
```
Une des deux classe manque cruellement d'accuracy, mais cela impacte moins l'accuracy global